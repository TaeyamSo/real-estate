...
export default function OwnersChapter() {
  return (
    <div
      id="owners"
    ...
    >
      <motion.div
        ...
        style={{
          backgroundImage: "linear-gradient(135deg, rgba(0,13,28,0.97) 0%, rgba(0,33,71,0.92) 100%), url('https://images.unsplash.com/photo-1560520653-9e0e4c89eb11?auto=format&fit=crop&w=1920&q=80')",
      ...
      <div className="relative z-10 text-center text-white px-8 max-w-4xl">
        ...
        >
          <motion.span className="block" initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ delayChildren: 0.4, staggerChildren: 0.12 }} variants={lineVariant}>
            {["Why", "Property", "Owners"].map((word) => (
              <motion.span key={word} className="inline-block mr-[0.25em]" variants={wordVariant}>{word}</motion.span>
            ))}
          </motion.span>
          <motion.span className="block" style={{ color: "#C5A021" }} initial="hidden" whileInView="visible" viewport={{ once: true }} transition={{ delayChildren: 0.8, staggerChildren: 0.12 }} variants={lineVariant}>
            {["Trust", "PNE"].map((word) => (
              <motion.span key={word} className="inline-block mr-[0.25em]" variants={wordVariant}>{word}</motion.span>
            ))}
          </motion.span>
